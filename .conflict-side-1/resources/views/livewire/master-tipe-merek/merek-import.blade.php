...
$import = function () {
    ...
    try {
        ...
        $this->reset();
        return $this->dispatch('infoNotifikasi', title: 'Merek', description: 'Data Merek berhasil diimport', icon: 'success');
    } catch (\Throwable $th) {
...
on([
    'downloadImport' => function () {
        try {
            $path = config('app.import_template_merek');
...
<div>
    <x-wireui-modal name="importModal"
                  blur="base">
    <x-wireui-card title="Import Merek">
